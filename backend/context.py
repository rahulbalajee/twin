...
try:
    ...
    raise RuntimeError(f"facts.json is missing required key: {e}") from e

facts_json = json.dumps(facts, indent=2, ensure_ascii=False)


def prompt() -> str:
    current_time = datetime.now(ZoneInfo("Asia/Kolkata")).strftime("%Y-%m-%d %H:%M:%S IST")

    linkedin_section = ""
    if linkedin:
        linkedin_section = f"""Here is the LinkedIn profile of {name}:
{linkedin}
...
{linkedin_section}Here are some notes from {name} about their communications style:
{style}
